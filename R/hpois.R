...
#'       P(X = x) = (1 - p_0)\,\frac{P_{\mathrm{Pois}}(x;\,\lambda)}{1 - e^{-\lambda}},
#'       \quad x = 1, 2, \ldots}
#'
...
#' \code{zeroprob} equals \eqn{e^{-\lambda}}.
#'
#' @references
...
dhpois <- function(x, lambda, zeroprob = 0.5, log = FALSE) {
  ...
  # the point mass at zero and the rescaled zero-truncated Poisson are combined
  # on the log scale, so that either branch may be exactly zero without a NaN
  log_zero <- log(zeroprob) + log(iszero(x))
  log_pos <- log1p(-zeroprob) + dpois(x, lambda, log = TRUE) -
    log1p(-exp(-lambda)) + log(ispos_strict(x))
