...
In each state, step lengths follow a gamma and turning angles a von
Mises distribution. Many of the elk’s turning angles are close to \pm
...
[`pvm()`](https://janolefi.github.io/RTMBdist/reference/vm.md) cuts the
circle open opposite to the mean direction, this is 1 for angles at the
mean direction and 0 for angles opposite to it, and it is uniformly
distributed. It is then joined with the step length by a standard
copula, here a Frank copula with state-dependent dependence parameter
\theta_j, while left and right turns are treated alike. Unlike a
Gaussian copula, the Frank copula’s density stays bounded everywhere on
the unit square, which avoids artificial spikes in the conditional
densities below.
[`dcopula()`](https://janolefi.github.io/RTMBdist/reference/dcopula.md)
...
nll_elk <- function(par) {
  ...
  kappa <- exp(log_kappa); REPORT(kappa) # angle concentrations around angle_mean
  REPORT(theta) # copula dependence parameters
  allprobs <- matrix(1, length(step), N)
  ind <- which(!is.na(step) & !is.na(angle))
  for(j in 1:N) {
    allprobs[ind, j] <- dcopula(
      dvm(angle[ind], angle_mean[j], kappa[j]), dgamma2(step[ind], mu[j], sigma[j]),
      pvm(angle[ind], angle_mean[j], kappa[j]), pgamma2(step[ind], mu[j], sigma[j]),
      copula = cfold(cfrank(theta[j]))
    )
  }
...
Fixing \theta_j = 0 via `map` gives the usual model with conditional
independence, which we fit for comparison. The Frank copula density has
a removable singularity exactly at \theta = 0, so we fix it at a value
...
par <- list(eta = rep(-2, 2), log_mu = log(c(0.3, 3)), log_sigma = log(c(0.3, 3)),
            angle_mean = c(pi, 0), log_kappa = log(c(0.3, 1)), theta = c(1e-6, 1e-6))
dat <- list(step = elk$step, angle = elk$angle, ID = elk$ID, N = N)
map <- list(angle_mean = factor(c(NA,NA)))

obj_ind <- MakeADFun(nll_elk, par, silent = TRUE,
                     map = c(map, list(theta = factor(c(NA, NA)))))
opt_ind <- nlminb(obj_ind$par, obj_ind$fn, obj_ind$gr)
obj_elk <- MakeADFun(nll_elk, par, map = map, silent = TRUE)
...
#> [1] 3769.963 3762.090
round(mod_elk$theta, 2)
#> [1] 0.89 0.51
```

The copula model has a clearly lower AIC. As fixing \theta_1 = \theta_2
= 0 gives the independence model, the models are nested and we can also
...
for(j in 1:N) { # turning angles in state j, for steps below and above the state's median
  cop <- cfold(cfrank(mod_elk$theta[j]))
  u <- pvm(angle_grid, par$angle_mean[j], mod_elk$kappa[j])
  sel <- ok & states == j
  ...
  for(g in 1:2) { # density of the angle given the step is short / long
    ...
    lines(angle_grid, c_avg * dvm(angle_grid, par$angle_mean[j], mod_elk$kappa[j]), col = cols[g], lwd = 2)
  }
...
for(j in 1:N) { # step lengths in state j, for angles far from / close to the state's mean direction
  cop <- cfrank(mod_elk$theta[j]) # after folding, the angle enters as w = 1 - |2u - 1|
  ...
  sel <- ok & states == j
  w <- 1 - abs(2 * pvm(elk$angle[sel], par$angle_mean[j], mod_elk$kappa[j]) - 1)
  close <- w > 0.5
...
Both dependence parameters are positive: the longer the step, the more
the turning angle concentrates around the state’s mean direction.
Travelling elk go straighter on long steps, and elk in state 1 turn back
more clearly on long steps, i.e. move back and forth.
