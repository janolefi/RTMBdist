...
We use the function `make_matrices()` from package `LaMa` (which uses `mgcv` internally, and which we call with `LaMa::` rather than attaching `LaMa`, as it also exports some distributions of the same names as `RTMBdist`) to conveniently create design and penalty matrices for the smooth functions.
The penalty matrix is converted to a sparse matrix using the `Matrix` package, to work with `RTMB`'s `dgmrf()` function.
For convenience, we also create a prediction design matrix for a sequence of age values, which we will use to get predictions and confidence intervals for the smooth functions. 
We bind the two matrices together to compute the prediction alongside the fitted values inside the likelihood. Note that this does not introduce overhead because `RTMB` ignores computations that do not contribute to the final function value.
...
Hidden Markov models (HMMs) for animal movement usually assume that step lengths and turning angles are independent given the behavioural state. Copulas let us drop this assumption. We use GPS tracks of four elk (Morales et al., 2004) from the `moveHMM` package and the forward algorithm from `LaMa` for the HMM likelihood.
...
In each state, step lengths follow a gamma and turning angles a von Mises distribution. Many of the elk's turning angles are close to $\pm \pi$, so we fix the mean turning angle at $\pi$ in state 1 (turning back, as when foraging in a patch) and at $0$ in state 2 (travelling).

As the turning angle is circular, standard copulas are not applicable. Angles of $-\pi$ and $\pi$ are the same, but would be opposite extremes for the copula. `cfold()` solves this by measuring each angle by $1 - |2u - 1|$, where $u$ is the angle's distribution function value. Since `pvm()` cuts the circle open opposite to the mean direction, this is 1 for angles at the mean direction and 0 for angles opposite to it, and it is uniformly distributed. It is then joined with the step length by a standard copula, here a Frank copula with state-dependent dependence parameter $\theta_j$, while left and right turns are treated alike. Unlike a Gaussian copula, the Frank copula's density stays bounded everywhere on the unit square, which avoids artificial spikes in the conditional densities below. `dcopula()` expects the angle as the first margin.
...
Fixing $\theta_j = 0$ via `map` gives the usual model with conditional independence, which we fit for comparison. The Frank copula density has a removable singularity exactly at $\theta = 0$, so we fix it at a value close to, but not exactly, zero.
...
The copula model has a clearly lower AIC. As fixing $\theta_1 = \theta_2 = 0$ gives the independence model, the models are nested and we can also use a likelihood ratio test:
...
To check the fit, we decode the states with the Viterbi algorithm and plot state-dependent histograms. To see the dependence, we split the observations of each state into two halves: the turning angles for steps shorter and longer than the state's median step length, and the step lengths for turning angles closer to and farther from the state's mean direction than its median. The lines are the corresponding densities implied by the model, which average the copula density over the half conditioned on.

```{r elk histograms, fig.width = 7, fig.height = 6}
...
Both dependence parameters are positive: the longer the step, the more the turning angle concentrates around the state's mean direction. Travelling elk go straighter on long steps, and elk in state 1 turn back more clearly on long steps, i.e. move back and forth.
```{r cleanup6, include=FALSE}
TMB::FreeADFun(obj_ind)
TMB::FreeADFun(obj_elk)
```
